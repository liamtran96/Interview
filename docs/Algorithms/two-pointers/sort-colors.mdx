...
Named after the Dutch flag (🇳🇱) which has three horizontal bands:
- Red (top) → 0s
- White (middle) → 1s
- Blue (bottom) → 2s
...
**Follow-up questions:**
- Why is it called Dutch National Flag? → Named after Dutch flag colors
- Can we extend this to k colors? → Yes, but more complex
- What if colors weren't 0,1,2? → Same algorithm, just adjust comparisons
- How would you test this? → Edge cases: all same color, already sorted, reverse sorted
